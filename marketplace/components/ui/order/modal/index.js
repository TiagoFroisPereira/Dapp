...
const createFormState = ({price, email, confirmationEmail}, hasAggreedTOS)  =>{
  ...
  }else if (!hasAggreedTOS){
    return _createFormState(true,"You need to agree with the terms of service in order to complete the form.")
  }
...
export default function OrderModal({course, onClose, onSubmit}){
  ...
  const [hasAggreedTOS, setHasAggreedTOS] = useState(false)
  ...
  const closeModal= () => {
    ...
    setHasAggreedTOS(false)
    onClose()
  }

  const formState = createFormState(order, hasAggreedTOS)
          ...
          <div className="sm:flex sm:items-start">
            <div className="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div className="text-xs text-gray-700 flex">
                <label className="flex items-center mr-2">
                  <input
                    checked={hasAggreedTOS}
                    onChange={({target: {checked}}) => {
                      setHasAggreedTOS(checked)
                    }}
                    type="checkbox"
                    className="form-checkbox" />
                </label>
